...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  uint64_t candidates[3] = {base == 0 ? base : base - (1UL << 32), base, base + (1UL << 32) == 0 ? base : base + (1UL << 32)};
  ...
  for (const uint64_t candidate : candidates) {
    ...
    uint64_t diff = (absolute_seqno < checkpoint) ? (checkpoint - absolute_seqno) : (absolute_seqno - checkpoint);
    if(diff < min_diff) { min_diff = diff; best_seqno = absolute_seqno; }
  }
  return best_seqno;
}
